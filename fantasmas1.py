...
class PacManGame:
    def __init__(self, map_data):
        ...
        self.ghost_x, self.ghost_y = self.find_ghost()
    ...
    def move_ghost(self):
        dx, dy = random.choice([(0, 1), (0, -1), (1, 0), (-1, 0)])  # Random ghost movement
        new_x = (self.ghost_x + dx) % len(self.grid[0])
        new_y = (self.ghost_y + dy) % len(self.grid)

        if self.grid[new_y][new_x] not in ['#', 'G']:
            self.grid[self.ghost_y][self.ghost_x] = ' '
            self.ghost_x, self.ghost_y = new_x, new_y
            self.grid[self.ghost_y][self.ghost_x] = 'G'

    def check_collision(self):
        if (self.pacman_x, self.pacman_y) == (self.ghost_x, self.ghost_y):
            self.game_over = True
    ...
    def draw_grid(self, screen):
        ...
        for y in range(len(self.grid)):
            for x in range(len(self.grid[y])):
                ...
                elif self.grid[y][x] == 'G':  # Use red color for the ghost
    ...
    def run(self):
        ...
        while not self.game_over:
            ...
            self.move_ghost()  # Move the ghost
            self.check_collision()  # Add this line to display the score
